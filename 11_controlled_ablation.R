...
# Decomposes the gap reported in the Results into single-factor comparisons.
# The protocol and features follow 10_replication_check.R (XGBoost + SMOTE + 70/30);
# the patient-level representative-species mapping is reused unchanged.
#   Gap 1 (unit / level attribution): the patient-level MDR ladder
#     Pt0 individual exposure only -> Pt1 + hospital-wide mapped pressure (cat_ddds) -> Pt2 + explicit species identity
#   Pt1 - Pt0 = clean marginal contribution of the pressure features to patient-level MDR
#   Gap 2: event-level MDR ablation (drop identity / drop identity + pressure),
#   benchmarked against the ablation already available for is_high_risk
# Output: output/ml/controlled_ablation.csv
...
# Outcome definitions identical to 10_replication_check.R
ev <- ev %>%
  mutate(is_high_risk = as.integer(
    str_detect(pathogen_name, "鲍曼不动") |
      str_detect(pathogen_name, "肺炎克雷伯") |
      str_detect(pathogen_name, "金黄色葡萄球|金黄葡萄球") |
...
# Quarterly hospital-wide consumption to ATC-class pressure (cat_ddds family)
...
# Representative species to associated ATC classes (priority picks the top species at patient level)
...
# Shared modelling protocol: only keep_feats enter the model; outcome columns never become features and all other columns are excluded.
# Per-sample labels/probabilities are stored at every step. Each run_protocol() call re-draws its own 70/30 split from its own outcome: a split is shared WITHIN a chain (same outcome, same labels) but not across chains, so every adjacent dAUC and its CI is a within-chain paired comparison; cross-chain magnitudes are not comparable.
# Without per-sample predictions the paired bootstrap / DeLong test is impossible; overlapping independent-sample CIs understate power.
PRED_STORE <- new.env(parent = emptyenv())
run_protocol <- function(d, outcome_name, keep_feats, tag = outcome_name) {
  y_all <- d[[outcome_name]]
  OUTCOME_NAMES <- intersect(names(d), c("is_high_risk", "mdr", "any_resistance"))
  x_all <- d %>% select(all_of(keep_feats)) %>% select(-any_of(OUTCOME_NAMES))
  xm <- model.matrix(~ . - 1, data = x_all)
  stopifnot(nrow(xm) == length(y_all))
  keep <- apply(xm, 2, sd) > 0
  xm <- xm[, keep, drop = FALSE]

  set.seed(2026)
  tr <- createDataPartition(y_all, p = 0.7, list = FALSE)
  Xtr <- xm[tr, ]; ytr <- y_all[tr]
  Xte <- xm[-tr, ]; yte <- y_all[-tr]
...
# Manual SMOTE, identical to 05_model_aggregate_level.R
  ...
  data.frame(outcome = outcome_name, n = nrow(xm),
             auc = as.numeric(pROC::auc(r)), ci_low = ci[1], ci_high = ci[3])
}

# Event-level MDR ablation
id_vars <- c("gram_class", "is_enterobacteriaceae", "is_nonfermenter")
ev_base_feats <- c(id_vars, "cat_ddds","cat_ddds_share","cat_amount_share",
                   "total_hosp_ddds","total_hosp_amount","specimen_type_std","season")
rows <- list()
rows[[1]] <- run_protocol(ev_model_df, "mdr", ev_base_feats, tag = "ev_mdr_base")
rows[[1]]$outcome <- "ev_mdr_base"
rows[[2]] <- run_protocol(ev_model_df, "mdr", ev_base_feats[!ev_base_feats %in% id_vars],
                          tag = "ev_mdr_abl_identity")
rows[[2]]$outcome <- "ev_mdr_abl_identity"
rows[[3]] <- run_protocol(ev_model_df, "mdr",
                          ev_base_feats[!ev_base_feats %in%
                                          c(id_vars, "cat_ddds","cat_ddds_share","cat_amount_share")],
                          tag = "ev_mdr_abl_identity_pressure")
rows[[3]]$outcome <- "ev_mdr_abl_identity_pressure"
rows[[4]] <- run_protocol(ev_model_df, "mdr",
                          c("total_hosp_ddds","total_hosp_amount","season"),
                          tag = "ev_mdr_abl_no_pressure")
rows[[4]]$outcome <- "ev_mdr_abl_no_pressure"

# Patient-level controlled MDR ladder
base <- readRDS("output/ml/patient_features.rds") %>%
  mutate(mrn = as.character(mrn),
         index_date = as.Date(index_date))
stopifnot(!any(duplicated(base$mrn)))

# Representative species = highest priority, as in 10_replication_check.R
mb2 <- mb
pat_map <- mb2 %>%
  mutate(pathogen_std = case_when(
    ...
    str_detect(pathogen_name, "嗜麦芽") ~ "嗜麦芽窄食单胞菌",
    TRUE ~ NA_character_)) %>%
  filter(!is.na(pathogen_std)) %>%
  left_join(pathogen_atc_map, by = "pathogen_std") %>%
  group_by(mrn) %>%
  slice_min(priority, n = 1, with_ties = FALSE) %>%
  ungroup() %>%
  select(mrn, pathogen_std, related_atc)

pat <- base %>%
  left_join(pat_map, by = "mrn") %>%
  mutate(
    mapped = as.integer(!is.na(related_atc)),
    season = case_when(
      month(index_date) %in% c(3,4,5) ~ "Spring",
      month(index_date) %in% c(6,7,8) ~ "Summer",
      month(index_date) %in% c(9,10,11) ~ "Autumn",
      month(index_date) %in% c(12,1,2) ~ "Winter",
      TRUE ~ "Unknown")) %>%
  left_join(atc_pressure, by = c("related_atc" = "atc_category")) %>%
  mutate(
    cat_ddds = ifelse(is.na(cat_ddds), 0, cat_ddds),
    ...
    cat_amount_share = ifelse(is.na(cat_amount_share), 0, cat_amount_share),
    gram_class = case_when(
      str_detect(pathogen_std, "葡萄球|肠球") ~ "Gram-positive",
      str_detect(pathogen_std, "大肠|克雷伯|鲍曼|铜绿|嗜麦芽") ~ "Gram-negative",
      TRUE ~ "Other"),
    is_enterobacteriaceae = as.integer(ifelse(is.na(str_detect(pathogen_std, "大肠|克雷伯")), FALSE,
                                              str_detect(pathogen_std, "大肠|克雷伯"))),
    is_nonfermenter = as.integer(ifelse(is.na(str_detect(pathogen_std, "鲍曼|铜绿|嗜麦芽")), FALSE,
                                        str_detect(pathogen_std, "鲍曼|铜绿|嗜麦芽"))))

own_rx <- c("ddds_J01C","ddds_J01D","ddds_J01G","ddds_J01X","ddds_J01M",
            "ddds_J02A","ddds_J01A","ddds_J01F","ddds_7d","ddds_14d","ddds_30d",
            "total_ddds_all","n_drugs","n_atc_cats","therapy_span_days","n_prescriptions")
cli <- c("n_specimens","n_pathogens","specimen_type_std","season")
pressure <- c("cat_ddds","cat_ddds_share","cat_amount_share")
identity <- c("gram_class","is_enterobacteriaceae","is_nonfermenter")

pat_df <- pat %>%
  mutate(specimen_type_std = factor(replace_na(specimen_type_std, "Other")),
         season = factor(replace_na(season, "Unknown")),
         gram_class = factor(replace_na(gram_class, "Other")))
# Numeric columns should have no NA; fill with 0 as a fallback.
for (cc in c(own_rx, cli, pressure, identity)) {
  if (is.numeric(pat_df[[cc]])) pat_df[[cc]] <- replace_na(pat_df[[cc]], 0)
}

keep_pt0 <- c(cli, own_rx)
rows[[5]] <- run_protocol(pat_df, "mdr", keep_pt0, tag = "pt_mdr_own_only")
rows[[6]] <- run_protocol(pat_df, "mdr", c(keep_pt0, pressure), tag = "pt_mdr_own_pressure")
rows[[7]] <- run_protocol(pat_df, "mdr", c(keep_pt0, pressure, identity),
                          tag = "pt_mdr_own_pressure_id")

tags <- c("ev_mdr_base", "ev_mdr_abl_identity", "ev_mdr_abl_identity_pressure",
          "ev_mdr_abl_no_pressure", "pt_mdr_own_only", "pt_mdr_own_pressure",
          "pt_mdr_own_pressure_id")
res <- bind_rows(rows)
res$outcome <- tags
write_csv(res, "output/ml/controlled_ablation.csv")
saveRDS(as.list(PRED_STORE)[tags], "output/ml/ablation_predictions.rds")

# Console summary (the CSV holds the complete results)
g <- function(nm) res$auc[res$outcome == nm]
cat(sprintf("ev MDR: base %.3f | -identity %.3f | -identity-pressure %.3f | -all %.3f\n",
            g("ev_mdr_base"), g("ev_mdr_abl_identity"), g("ev_mdr_abl_identity_pressure"),
            g("ev_mdr_abl_no_pressure")))
cat(sprintf("pt MDR: Pt0 %.3f -> Pt1 %.3f (+%.3f) -> Pt2 %.3f\n",
            g("pt_mdr_own_only"), g("pt_mdr_own_pressure"),
            g("pt_mdr_own_pressure") - g("pt_mdr_own_only"),
            g("pt_mdr_own_pressure_id")))
